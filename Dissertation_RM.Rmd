...
Figure \ref{fig:corr_plot} shows the correlation between the variables and shows that all the variables have a positive relationship with total score (*lsoatotscr*) to varying extents. For example, the degree educated variable (*dedrank*), and occupation variable (*occrank*), both have a more positive linear relationship with the total score, that (*carrank*), for example.

```{r corr_plot, fig.cap="Plot showing correlations between total scores and the 5 variables "}
zws_joined_lsoa_scr <- readRDS("zws_joined_lsoa_scr.rds")
a = as.data.frame(st_drop_geometry(zws_joined_lsoa_scr[,12:17]))
