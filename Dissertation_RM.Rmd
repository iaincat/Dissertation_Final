...
kable(st_drop_geometry(all_rcs[1:5,c(2:5)]), caption = "\\label{tab:rc_data}Retail centres data.")|>
  kable_styling(full_width = FALSE, latex_options = "HOLD_position", font_size = 8) |> row_spec(0,bold = T)
remove(all_rcs)
```
...
Distributions of all retail centre scores were then visualised, again using distibution histograms and can be seen in \ref{fig:all_rc_hists}.
...
The results of this study can be said to LINK TO THE LITERATURE.

What do chosen the variables say?

Are some variables more associated with the total score for each LSOA?
