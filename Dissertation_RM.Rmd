...
lsoas = lsoas |> select(-one_of('occscr', 'tenscr', 'carscr', 'pplscr', 'dedscr'))
...
#   geom_sf(data = pwc_clip, aes(geometry = pwcgeometry)) +
#   geom_sf(data = bishy, aes(geometry = store_location), colour = "red") +
...
# Test plot
  ...
  bishyjoined = as.data.frame(bishyjoined)
  bishyjoined <- st_transform(bishyjoined, crs = st_crs(3857))
###################################################################################
#  ggplot() +
#    annotation_map_tile(zoomin = 0, type = "hotstyle") +
#    geom_sf(data = bishyjoined, aes(geometry = bishyjoined$store_location)) +
#   geom_sf(data = ywalktime20, aes(geometry = ywalktime20$geometry), alpha = 0.1) #+
#    geom_sf(data = bishyjoined, aes(geometry = bishyjoined$joined_lsoas), alpha = 0.1)
# THIS WORKED
##################################################################################
...
##################################################################################
ggplot(rca_results[1,]) +
  annotation_map_tile(zoomin = 0, type = "hotstyle") +
  geom_sf(aes(geometry = rca_results$rcgeometry), alpha = 0.1) +
  geom_sf(aes(geometry = rca_results$walktimegeometry), alpha = 0.1) +
  geom_sf(aes(geometry = rca_results$rcwalktime), alpha = 0.1)


###################################################################################
# Join the rca-rcc_results data in to one file
rc <- cbind
