...
-   Can socio-demographic data (age, homeownership, vehicle access, occupation type (used as a proxy variable for income), and education level, be used to help in the understanding of the location of zero-waste stores?
-   Can the variables used in this study be used to provide evidence of the likely customer base of the stores in the catchment areas?
-   Are these variables able to be used to find gaps in potential market provision?

## Aims

There has been extensive research that concentrates on the importance of household waste, packaging, or the concept of zero-waste in general, and packaging in general, including its role in places such as zero-waste stores and supermarkets such as ([@scharpenberg2021]; [@sastre2022]; [@mollaloGISbased2020]). Demographic analysis is also a widely studied area in the social sciences and in retail more specifically. It's use by both of these areas of research, and in relation to the potential links between environmentally conscious behaviour and environmentally ethical store locations however, at the time of writing, is lacking, in that there are zero research studies relating to the locations of zero waste shops, and the possible associated demographics. This indicates an important gap in both retail (consumer and provider), and more generally, social, and behavioural, knowledge.

This study aims to...

-   Provide evidence of a customer base for this sector of retail, based on the demographic analysis of a defined catchment area
-   Provide the most likely optimal retail centre locations for new stores based on a demographic analysis

## Objectives

To meet the aim of the study, the following objectives should be completed: -

-   Create a scorecard for each area that is currently served by a zero waste store
-   Ascertain distributions of socio-demographics of the areas around the stores
...
AGE SHOULD ONT BE A VARIABLE AND SHOULD BE TESTED AFTER RESULTS HAVE BEEN SHOWN. THIS CAN HIGHLIGHT THE IMPORTANCE OF AGE RATHER THAN ARBITRARILY CHOOSING AN AGE A A VARIABLE TO INCLUDE IN THE SCORECARD.
...
```{r rc_minmax_hists, fig.height=4, fig.width=5, fig.cap="Histograms showing individual variable scores for the retail centres that score a total that falls within the minimum and maximum score range for the zero waste stores.", echo=FALSE}
# my_theme = readRDS("my_theme.rds")
# rc_joined_lsoa_scr <- readRDS("rc_joined_lsoa_scr.rds")
# zws_joined_lsoa_scr <- readRDS("zws_joined_lsoa_scr.rds")
# # rc_in_zws_mm <- readRDS("rc_in_zws_mm.rds")
...
```{r age_analysis}
# femaleages <- readRDS("femaleages.rds")
# my_theme <- readRDS("my_theme.rds")
...
# lsoas <- readRDS("lsoas.rds")
# ###################################################################################
# # For each records in allmatches, find the ages from femaleages and add this data to the allmatches data
# i = 0
# i = i + 1
# for (i in 1:nrow(allmatches)) {
# 
# # Find the PWCs in allmatches[i,] isochrone
# pwcinam = lsoa_pwc[allmatches[i,]$rcwalktime,]
#   
# # Find the matching lsoas
# lsoasinam = lsoas[(lsoas$LSOA21CD  %in% pwcinam$LSOA21CD),]  
#   
# # Find the age data for these lsoas
# agesinam = femaleages[(femaleages$mnemonic %in% lsoasinam$LSOA21CD),]
# 
# # Sum each age category in agesinam
#   agesinam <- agesinam |>
#    bind_rows(summarise(agesinam[3:45], across(where(is.numeric), sum),
#                        across(where(is.character), ~'Total')))
# 
# 
# 
...
# 
# ###################################################################################
# agesinamplotdata = as.data.frame(t(agesinam[10,3:45]))
# agesinamplotdata <- agesinamplotdata  |> dplyr::rename_at('10', ~'score')
# ggplot(agesinamplotdata, aes(x=agesinamplotdata$score) +
#   geom_histogram(breaks = seq(0, 10, by = 1), colour = "orange", fill = "darkgrey")) + my_theme
```
...
***The question as to whether socio-demographic variables that relate to EC behaviour can help to define optimal locations for ZWS will be discussed here. This section will discuss the various findings of this study and link them to the relevant literature. It will discuss the demographic variables in terms of their ability to provide evidence that justify store locations, the store locations and scores related to these variables, the retail centres and their scores and the possible optimal new stores locations based on variable scores.***
...
For example, the store with the highest score, which was the 'Refill Larder' sited in Teddington in the London borough of Richmond upon Thames, on a Small Local Retail Centre, in a relatively wealthy urban location. Contrasted with this was the store with the lowest overall score, which was found on the University of Leicester campus called 'NADA'. Although not in a town centre, or an urban residential location, it is supplied with inherent footfall.

This difference in score does not automatically equate to success or failure, and highlights potential issues in determining optimal locations based solely upon analysis of variables such as those used in this study.

This points to the potential disadvantages of analysing individual store locations over looking at average catchment area scores.

Without spending data it is difficult to say whether any store is more or less successful than any other, however, in terms of potential demand, the 'Refill Larder' may be more related to the variables used here, and may be more related to a combination of the variables, in which it scored highly across all five. The NADA store score does not score highly on any variable, but due to it's location, the score (especially for education level), becomes less relevant. Based on a university campus, it cannot be expected that the population are educated to degree level (yet). They are however on that path, and are therefore more likely to understand and engage with the environmental concepts involved in the ethos of zero waste stores.

### The Retail Centres

Discuss RCs in relation to the results
...
All retail centres would fall within the realms of a realistic location to place a new store. This is due somewhat to the size of these stores and the amount of demand that is required to maintain their ability to remain open. The results of this study show this, with ........................ hinting at the possibility that these stores can be situated in any location, whether the retail centre is a small local centre or a large retail centre. Without sales data however, this is difficult to confirm or deny.
