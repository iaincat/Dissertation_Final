...
```{r zw_scrs_hists, fig.cap="Histograms showing individual variable scores for the zero waste stores that score a total that matches the mean store total score."}
#zws_joined_lsoa_scr <- readRDS("zws_joined_lsoa_scr.rds")
...
```{r all_rc_hists, fig.cap="Histograms showing distributions for individual variable scores for all retail centres."}
# ###################################################################################
# # All rc_total scores
# rc_joined_lsoa_scr <- readRDS("rc_joined_lsoa_scr.rds")
...
```{r zwmeanrc_hists, fig.cap="Histograms showing total and individual variable scores for the retail centres that score a total which matches the mean store total score."}
...
```{r rc_iqr_hists, fig.cap="Histograms showing individual variable scores for the retail centres that score a total that falls within the interquartile range of the zero waste stores total scores."}
# rc_joined_lsoa_scr <- readRDS("rc_joined_lsoa_scr.rds")
# ###################################################################################
...
```{r rc_minmax_hists, fig.cap="Histograms showing individual variable scores for the retail centres that score a total that falls within the minimum and maximum score range for the zero waste stores."}
...
kable(t(a), caption = "\\label{tab:zws_scrsummary}Scorecard summary table for all zero waste stores.")|>
  kable_styling(full_width = FALSE,latex_options = c("HOLD_position"), font_size = 8)|>
  row_spec(0,bold = T, font_size = 12)
...
To be able to see clearly, how the zero waste store data scores were distributed, they were plotted as histograms. As figure \ref{fig:zws_scores_hist} shows, the *lsoatotscr* (total scores) were distributed normally between around 15 and 30 with a slight left skew. Both *pplrank* and *dedrank* show slight skewing to the right
...
Figure TABLE & MAP shows the store that scored the lowest of the 129, with a score of ??. As can be seen in the breakdown of the individual variables average scores, this area scored very low on all factors. This would at first glance suggest that it is not in the best location, however, the area it serves is highly residential
...
kable(minstore, row.names = F, caption = "\\label{tab:minstore_tab}Summary table for zero waste store with lowest score.")|>
...
wrap_plots(A = minstoremap,
           ...
           G = mintenscr,
           design = layout)
remove(minstore, minstoremap, minvarhead, mincarscr, mindedscr, minoccscr, minpplscr, mintenscr, layout)
```
...
kable(maxstore, row.names = F, caption = "\\label{tab:maxstore_tab}Summary table for zero waste store with highest score.")|>
  kable_styling(full_width = FALSE,latex_options = c("HOLD_position"), font_size = 10)|>
  column_spec(1,width = "0.8in", bold = T) |>
  column_spec(2,width = "0.8in") |>
  column_spec(3,width = "0.8in") |>
  ...
  column_spec(5,width = "0.8in") |>
  column_spec(6,width = "0.8in")
remove(zws_joined_lsoa_scr, maxstore)
```
...
wrap_plots(A = maxstoremap,
           ...
           G = maxtenscr,
           design = layout)
remove(maxstore, maxstoremap, maxvarhead, maxcarscr, maxdedscr, maxoccscr, maxpplscr, maxtenscr, layout)
```
