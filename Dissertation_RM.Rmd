...
ggplot() +
  ...
  geom_sf(data = bishy, aes(geometry = joinedlsoageometry, alpha = bishy$totalscore)) +
...
```{r echo=FALSE}
# zws_joined_lsoa_scr <- readRDS("zws_joined_lsoa_scr.rds")
# lower_bound <- median(zws_joined_lsoa_scr$lsoatotscr) - 3 * mad(zws_joined_lsoa_scr$lsoatotscr, constant = 1)
# lower_bound
# 
# upper_bound <- median(zws_joined_lsoa_scr$lsoatotscr) + 3 * mad(zws_joined_lsoa_scr$lsoatotscr, constant = 1)
# upper_bound
# 
# outlier_ind <- which(zws_joined_lsoa_scr$lsoatotscr < lower_bound | zws_joined_lsoa_scr$lsoatotscr > upper_bound)
# as.data.frame(outlier_ind)
# 
# zws_z_scores <- as.data.frame(scale(zws_joined_lsoa_scr$lsoatotscr))
# hist(zws_z_scores$V1)
# summary(zws_z_scores$V1)
# which(zws_z_scores$V1 < -3.29)
# 
# remove(zws_joined_lsoa_scr, zws_z_scores, outlier_ind, lower_bound, upper_bound)
```
...
Once comparative analysis was completed on the stores, the scorecards were then used to identify the most suitable potential retail centre(s) that were not already supplied by a zero waste store. This was done by narrowing down the retail centre options through the production and analysis of the distributed scores based on various metrics.

Firstly, using those RCs that scored a total that fell within the minimum and maximum store score ranges of the stores total scores were identified, and the distributions were investigated through the use of a summary table distribution histograms.

Secondly, the retail centres were narrowed down to include only those that had a total score that was within the interquartile range of the stores total scores.

Further filtering of the RCs was then done through identifying only those that scored a total that was the same as the average for the stores total mean score (24).

Finally, the final set of potential RCs were identified using the 5 variables and the total score of the stores data. Those RC variables that scored an average within +-1 of the same variable in the stores data were identified and only those that scored the same as the store mean score (24) for the total score were identified.
...
```{r rc_minmax_hists, fig.cap="Histograms showing individual variable scores for the retail centres that score a total that falls within the minimum and maximum score range for the zero waste stores.", fig.width = 5, fig.height = 4}
...
The ZWS location data was used to search the RC data, to identify which ZWS sat within one of the retail centres
...
The smallest difference between the individual scores in the these 2 stores is was seen in the household composition (*pplrank*) variable, with the lowest and highest scoring stores being 2 and 4 respectively. The vehicle access variable was also fairly low with the lowest to the highest scoring store being 5 and 8 respectively. All other variables were considerably higher for the store with the highest score, and the totals for each being 15 for the low scoring store and 32 for the highest store..

```{r minmaxcomp_plot, fig.cap="Histogram plot showing highest and lowest scoring stores variable and total scores"}
...
# # rename the columns
# plotcompare <- plotcompare  |> dplyr::rename_at('Store Name', ~'Store_Name')
# 
...
maxstoremap <- 
  ggplot(maxstore) +
  annotation_map_tile(zoomin = -1, type = "hotstyle") +
    geom_sf(data = rcmaxstr, aes(geometry = rcmaxstr$rcgeometry, shape = rcmaxstr$Classifica), alpha = 0.1, colour = "blue") +
...
The lowest scoring store as seen in \ref{fig:minstr_map} is located within the City of Coventry area which is in the West Midlands. The store was found not to be located in a retail centre, but appears to service a large residential area with little greenspace, and covers the whole of the city and its surrounding North-West districts.
...
minstoremap <- ggplot(minstore) +
  ...
  guides(colour = guide_legend(title = str_wrap(minstore$st_nam, width = 15), reverse = TRUE)) +
    geom_sf(data = rcminstr, aes(geometry = rcminstr$rcgeometry, shape = rcminstr$Classifica), alpha = 0.1, colour = "blue") +
  guides(shape = guide_legend(title = "RC Classification", reverse = TRUE)) +
  theme_void()
...
# l = as.data.frame(t(l))
# rc_scrs_summary <- data.frame(matrix(ncol = 6, nrow = 0))
...
would have thought pplscr would have been more important due to shared incomes and no children, but this did not seem to be very correaltd to increase in total scores
