...
zws = as.data.frame(zws)
# remove "geometry" column
zws = zws |> select(-one_of('geometry'))
# remove rows with no Lon
zws = zws |>  drop_na(Lon)
# convert Lon and Lat to a point geometry column
geomdf = zws |> 
  st_as_sf(coords = c("Lon", "Lat"), crs = 3857) |> 
  summarise(geometry = st_combine(geometry))  |> 
  st_cast("POINT")

# joint the zws and geometry column
zws = cbind(zws, geomdf)
remove(geomdf)

# convert zws to sf format
zws <- st_as_sf(zws)
...
kable(
  lsoas[1:4,c(2,3,9)],
  caption = "\\label{tab:lsoas}LSOA boundary data") |>
  kable_styling(
    full_width = FALSE, latex_options = "HOLD_position", font_size = 8) |>
  row_spec(0,bold = T)
```
...
-   TS003 - Household composition
    ...
    This dataset included those households that were classified as either, married single family households with no children, or, cohabiting couple families with no children.
...
This count was then converted to a percentage of the total number of households in that LOSA. Using the scaling method formula, as set out above, the final ranking scores were multiplied by 10 to provide an integer ranking for each LSOA. See Table \ref{tab:pplscr}.
...
pplscr = pplscr |> select(-one_of('score', 'score_calc', 'hhld_.', 'hhld_.', 'total_hhld'))

### Create the score
# Sum the relevant columns
pplscr$tothhld = rowSums(pplscr[,4:6])

#Create a % column from the sum
pplscr$hhldpcnt = (pplscr$tothhld/pplscr$Total..All.households)*100

# Use the scaling method to rank the % column 
pplscr$pplrank = round((pplscr$hhldpcnt - min(pplscr$hhldpcnt)) / (max(pplscr$hhldpcnt) - min(pplscr$hhldpcnt)), digits = 1)*10

# remove the unrequired columns
pplscr = pplscr |> select(-one_of('hhldpcnt', 'tothhld', 'Single.family.household..Cohabiting.couple.family..No.children', 'Single.family.household..Married.or.civil.partnership.couple..No.children', 'Single.family.household..All.aged.66.years.and.over', 'Total..All.households', 'X2021.super.output.area...lower.layer'))

# Rename the mnemonic column
pplscr <- pplscr  |>  rename_at('mnemonic', ~'LSOA21CD')

kable(pplscr[0:4,], caption = "\\label{tab:pplscr}Rankings for household composition data for England and Wales")|>
  kable_styling(full_width = FALSE,latex_options = c("HOLD_position"), font_size = 8)|>
  row_spec(0,bold = T) |>
  column_spec(1,width = "0.8in") |>
  column_spec(2,width = "0.5in")
...
# remove the unwanted columns
carscr = carscr |> select(-one_of('score', 'score_calc', 'X.'))

### Create the score
#Create a % column from the sum
carscr$hhldpcnt = (carscr$X1.car.or.van.in.household/carscr$Total..All.households)*100

# Use the scaling method to rank the % column 
carscr$carrank = round((carscr$hhldpcnt - min(carscr$hhldpcnt)) / (max(carscr$hhldpcnt) - min(carscr$hhldpcnt)), digits = 1)*10

# remove the unrequired columns
carscr = carscr |> select(-one_of('hhldpcnt', 'X1.car.or.van.in.household', 'Total..All.households', 'X2021.super.output.area...lower.layer'))

# Rename the mnemonic column
carscr <- carscr  |>  rename_at('mnemonic', ~'LSOA21CD')

kable(carscr[0:4,], caption = "\\label{tab:carscr}Rankings for vehicle access data for England and Wales")|>
  kable_styling(full_width = FALSE,latex_options = c("HOLD_position"), font_size = 8)|>
  row_spec(0,bold = T) |>
  column_spec(1,width = "0.8in") |>
  column_spec(2,width = "0.5in")
...
tenscr = tenscr |> select(-one_of('score', 'score_calc', 'X.', 'sum'))

### Create the score
# Sum the relevant columns
tenscr$tothhld = rowSums(tenscr[,4:5])

#Create a % column from the sum
tenscr$hhldpcnt = (tenscr$tothhld/tenscr$Total..All.households)*100

# Use the scaling method to rank the % column 
tenscr$tenrank = round((tenscr$hhldpcnt - min(tenscr$hhldpcnt)) / (max(tenscr$hhldpcnt) - min(tenscr$hhldpcnt)), digits = 1)*10


# remove the unrequired columns
tenscr = tenscr |> select(-one_of('hhldpcnt', 'tothhld', 'Owned..Owns.with.a.mortgage.or.loan', 'Owned..Owns.outright', 'Total..All.households', 'X2021.super.output.area...lower.layer'))

# Rename the mnemonic column
tenscr <- tenscr  |>  rename_at('mnemonic', ~'LSOA21CD')

kable(tenscr[0:4,], caption = "\\label{tab:tenscr}Rankings for homeownership data for England and Wales")|>
  kable_styling(full_width = FALSE,latex_options = c("HOLD_position"), font_size = 8)|>
  row_spec(0,bold = T) |>
  column_spec(1,width = "0.8in") |>
  column_spec(2,width = "0.5in")
...
-   TS067 - Highest level of qualification - degree educated
    ...
    Education level data was only included for those people that have degree level education or above. Again, a similar process to calculate the rankings was followed. The number of people in each LSOA that had a level of education at degree or above were summed, and this total was converted to a percentage of the total population for that LSOA. The ranking method was again applied to this percentage to provide the final integer ranking score for each LSOA. See Table \ref{tab:dedscr}.
...
# clean the unrequired columns and rename them for ease of use
dedscr = dedscr |> select(-one_of('score', 'X.', 'MinMax'))

#Create a % column from the sum
dedscr$hhldpcnt = (dedscr$degree_educated/dedscr$Total..All.usual.residents.aged.16.years.and.over)*100

# Use the scaling method to rank the % column
dedscr$dedrank = round((dedscr$hhldpcnt - min(dedscr$hhldpcnt)) / (max(dedscr$hhldpcnt) - min(dedscr$hhldpcnt)), digits = 1)*10

# remove the unrequired columns
dedscr = dedscr |> select(-one_of('hhldpcnt', 'degree_educated', 'Total..All.usual.residents.aged.16.years.and.over', 'X2021.super.output.area...lower.layer'))
...
kable(tenscr[0:4,], caption = "\\label{tab:dedscr}Rankings for degree education level or above data for England and Wales")|>
  kable_styling(full_width = FALSE,latex_options = c("HOLD_position"), font_size = 8)|>
  row_spec(0,bold = T) |>
  column_spec(1,width = "0.8in") |>
  column_spec(2,width = "0.5in")
...
-   TS063 - Occupation
    ...
    Occupation data included those people whose occupations were classified as managers, directors, senior officials, and professional occupations. The ranking was produced for this data in the same way as the occupationdata. See Table \ref{tab:occscr}.
...
occscr = occscr |> select(-one_of('X.', 'X..1', 'X..2', 'high_earners.', 'score', 'calc'))

### Create the score
# Sum the relevant columns
occscr$tothhld = rowSums(occscr[,4:5])

#Create a % column from the sum
occscr$hhldpcnt = (occscr$tothhld/occscr$Total..All.usual.residents.aged.16.years.and.over.in.employment.the.week.before.the.census)*100

# Use the scaling method to rank the % column 
occscr$occrank = round((occscr$hhldpcnt - min(occscr$hhldpcnt)) / (max(occscr$hhldpcnt) - min(occscr$hhldpcnt)), digits = 1)*10

# remove the unrequired columns
occscr = occscr |> select(-one_of('hhldpcnt', 'tothhld', 'X2..Professional.occupations', 'X1..Managers..directors.and.senior.officials', 'Total..All.usual.residents.aged.16.years.and.over.in.employment.the.week.before.the.census', 'X2021.super.output.area...lower.layer'))
...
kable(occscr[0:4,], caption = "\\label{tab:occscr}Rankings for occupation data for England and Wales")|>
  kable_styling(full_width = FALSE,latex_options = c("HOLD_position"), font_size = 8)|>
  row_spec(0,bold = T) |>
  column_spec(1,width = "0.8in") |>
  column_spec(2,width = "0.5in")
...
# Join each variables score data to the lsoas data and remove the unrequired columns each time
# carscr
lsoas = merge(x = lsoas, y = carscr, all.x = TRUE)

# dedscr
lsoas = merge(x = lsoas, y = dedscr, all.x = TRUE)

# occscr
lsoas = merge(x = lsoas, y = occscr, all.x = TRUE)

# pplscr
lsoas = merge(x = lsoas, y = pplscr, all.x = TRUE)

# tenscr
lsoas = merge(x = lsoas, y = tenscr, all.x = TRUE)

# Save the lsoas dataframe
saveRDS(obj = lsoas, file = "lsoas.rds")

# sum the individual LSOA row rankings in columns 10 to 14 and add the total ranking result to a new column at the end
lsoas = lsoas |>
...
```{r bishy_load}
# Load bishy store boundary shapefile as a dataframe
# bishy <- as.data.frame(readRDS("bishy.rds"))
bishy <- as.data.frame(readRDS("bishy.rds"))

# clean the file
bishy = bishy |> select(-one_of('joinedlsoageometry', 'dedscr', 'pplscr', 'carscr', 'tenscr', 'occscr', 'bishygeometry'))
...
bishy = cbind(bishy, geomdf)
remove(geomdf)
# rename the geometry column
bishy <- bishy  |>  rename_at('geometry', ~'bishystorepoint')
...
# add the isochrone data to the bishy file
bishy = as.data.frame(bishy)
bishy = cbind(bishy, ywalktime20[4])

# rename this geometry to a meaningful name
bishy <- bishy  |>  rename_at('geometry', ~'bishyisochrone')

# convert bishy to an sf format
bishy = st_as_sf(bishy)
bishy <- st_transform(bishy, crs = st_crs(4326))
remove(ywalktime20)
# Save the York isochrone data
#saveRDS(obj = ywalktime20, file = "ywalktime20.rds")
```

Once the isochrone had been calculated from the test store, it was mapped to ensure that the correct location and catchment area had been calculated for the the known store data. See Figure \ref{fig:york_isochrone_test_map}.
...
ggplot(data = bishy$bishyisochrone, aes(colour = "20 minutes walk time")) +
...
```{r york_pwc_clipping}
# Convert the lsoas data to an sf format
lsoas = st_as_sf(lsoas)
lsoas <- st_transform(lsoas, crs = st_crs(3857))
# clip the PWCs to only show those inside the 20 minute walk time
pwc_clip = lsoa_pwc[bishy$bishyisochrone,]
# clip the LSOAs to only show those for the clipped PWCs
lsoa_clip = lsoas[pwc_clip,]

# save the lsoa_clip file for Nick to look at
#saveRDS(obj = lsoa_clip, file = "lsoa_clip.rds")
#saveRDS(obj = lsoa_clip, file = "lsoa_clip.rds")
...
Once the variables scores for the test store had each been checked for validity, a final total mean score for the variables and the area as a whole was calculated by summing the scores for each variable and dividing this by the number of LSOAs in the area. This method was used to maintain equivalence and comparative ability between the scorecard results of each store in the data. This was because some stores may have greater or fewer LSOAs within the catchment area for example, rendering them isolated in terms of data without comparison. See \ref{fig:ytotalscore_map}.
...
joined_lsoas <- joined_lsoas  |>  rename_at('geometry', ~'joinedlsoageometry')  

# calculate total score for all joined lsoas in the area
a = sum(round(lsoa_clip_tot[,14]/nrow(lsoa_clip_tot)), digits = 0)

# Join the joinedlsoa geometry and the total score for the area to the bish data
bishy = cbind(bishy, joined_lsoas, a)
# rename the a column
bishy <- bishy  |>  rename_at('a', ~'totalscore')  

# remove the not required dataframes etc
remove(a, joined_lsoas, lsoa_clip, lsoa_clip_tot, pwc_clip)

# Save the bishy data
saveRDS(obj = bishy, file = "bishy.rds")
```

```{r ytotalscore_map, warning=FALSE, fig.cap="Map showing the total mean score for the total of the 5 variables for each individual LSOA once combined."}
ggplot() +
    # Add the basemap tile 
  annotation_map_tile(zoomin = 0, type = "osm") +
  # total score for LSOA layer
  geom_sf(data = bishy, aes(geometry = bishy$joinedlsoageometry, fill = totalscore)) +
  # fill layer
...
```{r}
# remove the test store temporary dataframes
remove(bishy, lsoa_clip, pwc_clip)
```
...
# zws <- read_sf('C:/Users/James/Desktop/Diss_data_final/zws.shp')
# saveRDS(obj = zws, file = "zws.rds")
zws <- readRDS("zws.rds")
#zwsiso <- readRDS("zwsiso.rds")
#zws <- st_transform(zws, crs = st_crs(3857))
# 
# # Loops through the zws file and creates a new dataframe (zwsiso) that holds the zws store data and the isochrone for the walktime20 mins boundary.
# 
# #############################################################################
# #                                                                           #
...
zwsiso = zwsiso |> select(-one_of('group_index', 'center', 'value'))
# Save the zwsiso file
saveRDS(obj = zwsiso, file = "zwsiso.rds")
remove(i, isodf, zws, coordinates)
```
...
ggplot() +
  annotation_map_tile(zoomin = 0, cachedir = system.file("rosm.cache", package = "ggspatial")) +
...
ggplot(zwsiso[1,]) +
  ...
  geom_sf(aes(geometry = zwsiso$zwsgeometry)) +
  geom_sf(aes(geometry = zwsiso$walktimegeometry), alpha = 0.1)
```

```{r}
zwsiso <- readRDS("zwsiso.rds")
ggplot(zwsiso[1,]) +
  geom_sf(aes(fill = st_nam)) +
  geom_sf(aes(geometry = walktimegeometry), alpha = 0.1) +
  annotation_map_tile(zoomin = 5, type = "osm")
```
...
for (i in 1:nrow(zwsiso)) {
 ...
 a = lsoa_pwc[zwsiso[i,]$walktimegeometry,]
  ...
  # Check if (a) has 0 rows. If so, remove it and move to the next record
 if(nrow(a) == 0) {
   next
 }
...
# calculate total score for all joined lsoas in the area
c = round(b[nrow(b),9:14], digits = 0)
...
# Join the lsoas to a single shape so that a single line for this shape with the averages score can be used.
 ...
 # joined_lsoas_b = cbind(joined_lsoas, b[nrow(b),9:14])
  joined_lsoas_b = cbind(joined_lsoas, c)
  # delete the lsoa geometry column as this is multipolygon rather than the required single ploygon
 ...
 d = zwsiso[i,]
 d$st_nam <- paste(d$st_nam, d$town, sep=" - ")
 ...
 remove(a, b, c, d, e, joined_lsoas, joined_lsoas_b, r)
...
Of the original STORES 140 stores WERE PROCESSED AND OF THOSE , 130 were successfully processed WITH VALID ISOCHRONES AND SCORES. Those that were not, either did not produce isochrone calculations, with PWCs situated inside them, or did not have valid coordinate geometry data, as they were stores that closed prior to the study.

The results for the remaining 130 stores were analysed to calculate the mean scores of the 5 variables. The total mean score was then calculated to give the final scorecard result. These scorecards were then used to analyse the locations that currently contain a store, and to identify the most suitable potential retail centre that was not already supplied by a zero waste store.
...
```{r all_zws_scr_table}
kable(zws_joined_lsoa_scr[c(1:5),c(1, 2, 11:16)], caption = "\\label{tab:allzwsscrtable}Scorecard table for first 10 zero waste stores")|>
  kable_styling(full_width = FALSE,latex_options = c("HOLD_position"), font_size = 8)|>
...
cor_plot <- plot(zws_joined_lsoa_scr[,c(11:16)], cex = 0.3, 
     col = grey(0.145,alpha=0.5), upper.panel=panel.smooth)
```

```{r totscr_hist}
h1 <- ggplot(zws_joined_lsoa_scr, aes(x=lsoatotscr)) + 
  geom_histogram(breaks = seq(0, 50, by = 2), colour = "orange", fill = "darkgrey")
```

```{r carscr_hist}
h2 <- ggplot(zws_joined_lsoa_scr, aes(x=carrank)) + 
  geom_histogram(breaks = seq(0, 10, by = 1), colour = "orange", fill = "darkgrey")
```

```{r dedscr_hist}
h3 <- ggplot(zws_joined_lsoa_scr, aes(x=dedrank)) + 
  geom_histogram(breaks = seq(0, 10, by = 1), colour = "orange", fill = "darkgrey")
```

```{r pplscr_hist}
h4 <- ggplot(zws_joined_lsoa_scr, aes(x=pplrank)) + 
  geom_histogram(breaks = seq(0, 10, by = 1), colour = "orange", fill = "darkgrey")
```

```{r tenscr_hist}
h5 <- ggplot(zws_joined_lsoa_scr, aes(x=tenrank)) + 
  geom_histogram(breaks = seq(0, 10, by = 1), colour = "orange", fill = "darkgrey")
```

```{r occscr_hist}
h6 <- ggplot(zws_joined_lsoa_scr, aes(x=occrank)) + 
  geom_histogram(breaks = seq(0, 10, by = 1), colour = "orange", fill = "darkgrey")
```
...
maxstore$zwsgeometry <- st_transform(maxstore$zwsgeometry, crs = st_crs(3857))
maxstore$walktimegeometry <- st_transform(maxstore$walktimegeometry, crs = st_crs(3857))
maxstore$zwsjlsoageometry <- st_transform(maxstore$zwsjlsoageometry, crs = st_crs(3857))
```

The area that scored the highest overall can be seen in figure \ref{fig:maxstr_map}.

```{r}
# maxstore = as.data.frame(maxstore)
# maxstore = st_as_sf(maxstore)
maxstoremap <- 
  ggplot(maxstore) +
  annotation_map_tile(zoomin = 0, type = "osm") +
